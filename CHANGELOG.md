...
- **增强型日志系统**: 实现了按日期自动切分的日志系统 (`logs/tars-YYYY-MM-DD.log`)，并支持静默记录详细的 Traceback 和搜索调试快照。

### 🛠️ 技术优化
- **UI/UX 改进**: 控制台不再显示冗长的报错堆栈，改为简洁的错误提示 + 后台日志详细记录。
- **JSON Schema 约束**: 通过技能声明文件强制模型生成符合规范的参数。
- **容器环境优化**: 完善了 `docker-compose.yml` 的挂载配置，支持日志和技能目录的实时同步。
